	...
	return new Promise((resolve,reject) => {
		if (isMainThread)
		{
			try{
				...
				resizeWorker.on('message',(message)=>{resizeWorkerFinished = true;
				...
				resizeWorker.on('exit',(code)=>{if (code !== 0) reject(new Error('Exited with status code '+code));});
				
				monochromeWorker.on('message',(message) => {monochromeWorkerFinished = true;
				...
				monochromeWorker.on('exit',(code) => {if (code !== 0) reject(new Error('Exited with status code '+code));});
